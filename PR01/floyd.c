...
void build_table()
{
    ...
    // crear encabezados de columnas primero
    for (int j = 0; j < current_n; j++)
    {
        GtkWidget *entry = gtk_entry_new();
        char buf[8]; snprintf(buf, sizeof(buf), "%c", 'A' + j);
    ...
    {
        GtkWidget *entry = gtk_entry_new();
        char buf[8]; snprintf(buf, sizeof(buf), "%c", 'A' + i);
        gtk_entry_set_text(GTK_ENTRY(entry), buf);
        gtk_widget_set_size_request(entry, 60, -1);
        gtk_grid_attach(GTK_GRID(grid_table), entry, 0, i + 1, 1, 1);
        name_row[i] = entry;

        // conectar señales de sincronización (ambas direcciones)
        ...
        for (int j = 0; j < current_n; j++)
        {
            ...
            gtk_widget_set_size_request(e, 80, -1);
            if (i == j)
            {
                gtk_entry_set_text(GTK_ENTRY(e), "0");
                gtk_editable_set_editable(GTK_EDITABLE(e), FALSE); // diagonal fija
            }
            else
            {
...
{
    if (!txt) return INF;
    if (strcasecmp(txt, "INF") == 0 || strlen(txt) == 0) return INF;
    // permitir espacios
    char *end; long v = strtol(txt, &end, 10);
    if (end == txt) return INF;
    return (int)v;
}
...
char *build_route_string(int i, int j, int **P, char names[][32], int n) {
    ...
    while (sstart < send) {
        ...
        int a = cur.a, b = cur.b;
        int k = P[a][b];
        if (k == -1) { // si el camino es directo
            // agrega el primer nodo
            if (nodes_len == 0 || nodes[nodes_len-1] != a) nodes[nodes_len++] = a;
            // agrega el segundo nodo
            if (nodes_len == 0 || nodes[nodes_len-1] != b) nodes[nodes_len++] = b;
        } else {
            // si hay parada entre los nodos divide en dos para agregar cada segmento
            // cuidar si n es mayor a 8 puede fallar
            for (int t = send+1; t > sstart; t--) segs[t] = segs[t-1];
            segs[sstart] = (Seg){a, k};
...
void write_tex(const char *fname, int n, int ***snapD, int ***snapP, char names[][32], int steps) {
    ...
    fprintf(f, "}\n");
    fprintf(f, " & ");
    for (int j=0;j<n;j++) { fprintf(f, "%s", names[j]); if (j<n-1) fprintf(f, " & "); }
    fprintf(f, " \\\\ \\hline\n");
    for (int i=0;i<n;i++) {
        fprintf(f, "%s & ", names[i]);
        for (int j=0;j<n;j++) {
            int val = snapD[0][i][j];
            if (val >= INF) fprintf(f, "$\\infty$"); else fprintf(f, "%d", val);
            if (j<n-1) fprintf(f, " & ");
        }
        fprintf(f, " \\\\\n");
    }
    ...
    for (int i=0;i<n;i++) {
        ...
        for (int j=0;j<n;j++) {
            int v = snapP[0][i][j];
            if (v == -1) fprintf(f, "0"); else fprintf(f, "%s", names[v]);
            if (j<n-1) fprintf(f, " & ");
        }
        fprintf(f, " \\\\\n");
    }
    ...
    for (int k=1;k<steps;k++) {
        ...
        for (int i=0;i<n;i++) {
            ...
            for (int j=0;j<n;j++) {
                ...
                int cur = snapD[k][i][j];
                if (cur != prev) { // revisa si hay un cambio en D para escribirlo en tabla P
                    if (cur >= INF) fprintf(f, "\\textcolor{orange}{$\\infty$}");
                    else fprintf(f, "\\textcolor{orange}{%d}", cur);
                } else {
                    if (cur >= INF) fprintf(f, "$\\infty$"); else fprintf(f, "%d", cur);
                }
                if (j<n-1) fprintf(f, " & ");
            }
            fprintf(f, " \\\\\n");
        ...
        for (int i=0;i<n;i++) {
            ...
            for (int j=0;j<n;j++) {
                int v = snapP[k][i][j];
                if (v == -1) fprintf(f, "-"); else fprintf(f, "%s", names[v]);
                if (j<n-1) fprintf(f, " & ");
            }
            fprintf(f, " \\\\\n");
        }
...
void run_floyd(GtkWidget *btn, gpointer data)
{
    ...
    for (int i=0;i<n;i++) {
        ...
        if (!t || strlen(t)==0) snprintf(names[i], sizeof(names[i]), "%c", 'A'+i);
        else { strncpy(names[i], t, sizeof(names[i])-1); names[i][31]=0; }
    }
